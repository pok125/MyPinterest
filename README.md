...
username을 처리하는 부분에서 어려움을 겪었습니다. 이 과정에서 AbstractUser모델을 상속받아 커스텀 하려하면 User모델 뿐만아니라 UserManager class 또한 수정해야 한다는 것을 알게되었습니다.
...
2. Django ORM 이해
  * 쿼리 셋에 대해 실질적인 쿼리가 어떻게 수행되는지 알아보기 위해 logging과 pygments을 사용하였습니다. 실질적으로 쿼리문이 찍히는 것을 보니 기존에 생각하고 있던 실행 시점과 동작 방식이 전혀 다르다는 것을 알 수 있었습니다.
  ```
  a = 모델.objects.all()
  b = a[0]
  c = list(a)
  ```
위의 코드에서 all을 했을 때 db에서 모든 데이터를 가져오는 줄 알았지만 실질적으로 사용을 할 때 쿼리문이 동작하는 것을 알 수 있었습니다. 또한, b와 c 위의 코드에서 b, c에서 두 번의 쿼리가 발생하게 됩니다. 하지만 c, b를 위치를 바꾸게 되면 
쿼리문은 c에서 한 번 발생합니다. 따라서 쿼리를 통해 가져온 데이터가 캐싱된다는 것을 확인할 수 있었습니다. 이러한 이해를 토대로 작성했었던 코드들을 다시 살펴보았고 N+1 쿼리가 발생하는 부분들을 발견하여 annotate와 select_related를 이용하여 
해결하였습니다. 그 밖에도 사용할 수 있는 함수들이 많이 있다는 것도 알게 되었습니다. 데이터가 적기 때문에 복잡한 쿼리문으로 쿼리 발생 빈도를 줄이는 것과 단순한 쿼리 여러 번으로 처리하는 것에 대해 시간차이는 경험할 수 없었지만 장고의 쿼리 동작에 대해 
이해할 수 있었습니다.

3. Docker를 이용한 배포
  * 웹 서비스 배포는 처음이었기 때문에 한 단계씩 확장해 나가는 방식으로 배포를 진행하였습니다.
  * 첫째, 간단하게 DockerFile을 작성하여 Django 컨테이너를 만들어 서버가 동작하는지 확인하였습니다. GitHub의 public저장소에 코드를 clone 하여 단순히 runserver를 실행시켜 동작하는지 확인한 결과 
페이지가 동작했지만 css, js, image 파일들과 같은 정적인 파일들을 불러오지 못하는 문제가 발생하였고 Django 문서에서 runserver를 이용한 배포는 권장하지 않는다고 명시되어 있었습니다.
  * 둘째, Nginx와 gunicorn을 이용하여 첫 번째 방법의 문제를 해결하였습니다. 이 과정에서 정적인 데이터에 대한 요청은 웹 서버인 Nginx가 처리하고 그 외의 요청은 Nginx가 Django서버에 요청하는 구조를 이해했습니다. 
또한, Django서버의 static과 media 데이터를 Nginx 컨테이너와 공유하며 동기화하는 작업이 필요했기에 Docker Volume을 이용하여 각 컨테이너 간 데이터 공유와 동기화를 해결하였습니다.
  * 셋째, 재배포를 할 경우, 컨테이너들을 다시 만들 때 각 컨테이너에 대해 초기 세팅 값을 계속 설정해줘야 한다는 번거로움이 있었습니다. 따라서 Docker Stack을 이용하여 각 컨테이너들의 초기 세팅 값들을 명시하는 yml파일을 
작성하여 하나의 파일로 관리되도록 해결하였고 컨테이너들이 종료되었을 때 자동 재시작이 되도록 서비스형태로 관리하였습니다.
  * 넷째, GitHub의 public저장소를 private으로 변경하는 작업을 진행하였습니다. private으로 변경하며 git clone을 할 때 권한에 대해 입력해야 하는 문제가 발생하였습니다. 따라서 ssh-rsa 키 값을 생성하여 GitHub 아이디에 
등록 후 DockerFile에 rsa 키 값을 확인 후 git clone을 진행하도록 하였습니다.
  * 한 단계씩 확장해 나가는 방식을 통해 CI/CD에 대해 고민 해보는 시간을 가졌고 중요성 또한 경험할 수 있었습니다.
