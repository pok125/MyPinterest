...
3. Docker를 이용한 배포
  ...
  * 첫째, 간단하게 DockerFile을 작성하여 Django 컨테이너를 만들어 서버가 동작하는지 확인하였습니다. github의 public저장소에 코드를 clone하여 단순히 runserver를 실행시켜 동작하는지 확인한 결과
  페이지가 동작했지만 css, js, image 파일들과 같은 정적인 파일들을 불러오지 못하는 문제가 발생하였고 Django 문서에서 runserver를 이용한 배포는 권장하지 않는다고 명시되어있었습니다.
  * 둘째, Nginx와 gunicorn을 이용하여 첫번째 방법의 문제를 해결하였습니다. 이 과정에서 정적인 데이터에 대한 요청은 웹 서버인 nginx가 처리하고 그 외의 요청은 nginx가 Django서버에 요청하는 구조를 이해했습니다. 
  또한, Django서버의 static과 media 데이터를 nginx 컨테이너와 공유하며 동기화하는 작업이 필요했기에 Docker Volume을 이용하여 각 컨테이너간 데이터 공유와 동기화를 해결하였습니다.
  * 셋째, 재배포를 할 경우, 컨테이너들을 다시 만들때 각 컨테이너에 대해 초기 세팅 값을 계속 설정해줘야한다는 번거로움이 있었습니다. 따라서 Docker Stack을 이용하여 각 컨테이너들의 초기 세팅 값들을 명시하는 yml파일을
  ...
  * 한 단계씩 확장해 나가는 방식을 통해 CI-CD에 대해 고민해보는 시간을 가졌고 중요성 또한 경험할 수 있었습니다.
